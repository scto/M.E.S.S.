...
public class SparkBlue implements Poolable {
	
	private static final float WIDTH = CSG.screenWidth / 200, HALF_WIDTH = WIDTH/2;
	private static final float MIN_WIDTH = Stats.u;
	private static final Pool<SparkBlue> POOL_STANDARD = new Pool<SparkBlue>(100) {
		@Override
		protected SparkBlue newObject() {
			return new SparkBlue();
		}
	};
	private float x, y;
	private final float speedX, speedY, angle, height, halfHeight;
	private int ttl;
	protected final float color = AssetMan.convertARGB(1, CSG.R.nextFloat()/8, (CSG.R.nextFloat() + .8f) / 1.6f, .85f);
	...
	public static void act(Array<SparkBlue> explosions, SpriteBatch batch) {
		if (EndlessMode.alternate) {
			for (final SparkBlue p : explosions) {
	...
	public static void clear(Array<SparkBlue> explosions) {
