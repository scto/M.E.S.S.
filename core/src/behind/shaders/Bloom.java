...
public final class Bloom implements Serializable {
	...
	/**
	 * To use implement bloom more like a glow. Texture alpha channel can be used as mask which part are glowing and which are not. see more info at:
	...
	private ShaderProgram tresholdShader, bloomShader, blurShader;

	private Mesh fullScreenQuad;

	private Texture pingPongTex1, pingPongTex2, original;

	private FrameBuffer frameBuffer, pingPongBuffer1, pingPongBuffer2;

	private float bloomIntensity, originalIntensity, treshold;
	private int w, h;
	private boolean blending = false, capturing = false, disposeFBO = true;
	...
	/**
	 * Initialize bloom class that capsulate original scene capturate, tresholding, gaussian blurring and blending.		 Default values: depth = true 		blending = false 		32bits = true
	 */
	public Bloom() {
	...
	/**
	 * EXPERT FUNCTIONALITY. no error checking. Use this only if you know what you are doing. Remember that bloom.capture() clear the screen so use
	...
	private void initialize(int FBO_W, int FBO_H, FrameBuffer fbo, boolean hasDepth, boolean useBlending, boolean use32bitFBO) {
		...
		if (use32bitFBO) {
			if (useBlending)		format = Format.RGBA8888;
			else 					format = Format.RGB888;
		} else {
			if (useBlending)		format = Format.RGBA4444;
			else					format = Format.RGB565;
		}
		if (fbo == null)			frameBuffer = new FrameBuffer(format, Gdx.graphics.getWidth(), Gdx.graphics.getHeight(), hasDepth);
		else						frameBuffer = fbo;
		...
		if (useAlphaChannelAsMask)		tresholdShader = BloomShaderLoader.createShader("screenspace", "maskedtreshold");
		else							tresholdShader = BloomShaderLoader.createShader("screenspace", alpha + "treshold");
